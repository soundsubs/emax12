...
audio_fx_api_v2_t* move_audio_fx_init_v2(const host_api_v1_t *host) {
    ...
    /* NOTE: a prior version of this function also logged host->api_version,
     * host->sample_rate, host->frames_per_block, etc. That log line NEVER
     * appeared in debug.log across multiple tests, even though this line
     * (before it) did -- strong evidence the plugin crashes the instant it
     * touches those struct fields, consistent with audio_fx_api_v2.h being
     * a hand-transcribed (not authoritative) struct layout. Reverted to
     * avoid crashing further while this gets sorted out via the real
     * module authors (see README's "Known issue" section). */
    if (!host) {
